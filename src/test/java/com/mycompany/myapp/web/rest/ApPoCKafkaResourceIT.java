...
class ApPoCKafkaResourceIT {
    ...
    void setup() {
        ...
        ApPoCKafkaResource kafkaResource = new ApPoCKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ap-po-c-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/ap-po-c-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
